...
def get_file_content_from_dataset_folder(
  ...
  Args:
    clz: The dataset collection class for which the file has to be read.
  ...
  Returns:
    ...
    encountered error.
  """
  file_path = get_filepath_in_dataset_folder(clz, file_name)
  try:
...
class DatasetCollectionInfo:
  ...
  @classmethod
  def from_cls(cls,
               dataset_collection_cls: Type["DatasetCollection"],
               release_notes: Mapping[str, str],
               description: Optional[str] = None,
               citation: Optional[str] = None) -> "DatasetCollectionInfo":
    """Creates a DatasetCollectionInfo instance based on class information."""
    name: str = naming.camelcase_to_snakecase(dataset_collection_cls.__name__)
    if not description:
      description = get_file_content_from_dataset_folder(
          dataset_collection_cls, DESCRIPTION_FILE, raise_error_if_fails=True)
    if not citation:
      citation = get_file_content_from_dataset_folder(dataset_collection_cls,
                                                      CITATIONS_FILE)
    return cls(
        name=name,
...
class DatasetReference:
  ...
  Attributes:
    ...
    split_mapping: mapping between split names. If the `DatasetCollection` wants
      to use use different split names than the source datasets, then this
      mapping can be used. For example, if the collection uses the split
      `valid`, but this dataset uses the split `validation`, then the
      `split_mapping` should be `{'validation': 'valid'}`.
...
def references_for(
  ...
  """Simple construction of dataset references.
  ...
  Args:
    name_to_tfds_name: mapping between name to be used in the dataset collection
      and the TFDS name (plus optional config and version).
...
class DatasetCollection(
  ...
  It expects subclasses to overwrite `info` to return populated
  DatasetCollectionInfo.

  It also expect subclasses to overwrite `datasets` to return a dictionary of
  versions to the datasets included in that collection's version.
  """
