...
def test_get_file_content_from_dataset_folder2():
  with mock.patch.object(
      dcb, 'get_filepath_in_dataset_folder', autospec=True) as mock_get_path:
    ...
    actual = dcb.get_file_content_from_dataset_folder(
        clz=None, file_name='anything')
    mock_get_path.assert_called_once_with(None, 'anything')
    assert actual == expected


def test_get_file_content_from_dataset_folder_raise_exception():
  with pytest.raises(AttributeError):
    with mock.patch.object(
        dcb, 'get_filepath_in_dataset_folder', autospec=True) as mock_get_path:
      mock_get_path.return_value = FileNotFoundError
      dcb.get_file_content_from_dataset_folder(
          clz=None, file_name=None, raise_error_if_fails=True)


def test_get_file_content_from_dataset_folder_return_none():
  with mock.patch.object(
      dcb, 'get_filepath_in_dataset_folder', autospec=True) as mock_get_path:
    mock_get_path.return_value = FileNotFoundError
    actual = dcb.get_file_content_from_dataset_folder(clz=None, file_name=None)
...
def test_dataset_collection_info_from_cls():
  ...
  dummy_dc_info = dcb.DatasetCollectionInfo.from_cls(
      dataset_collection_cls=testing.DummyDatasetCollection,
